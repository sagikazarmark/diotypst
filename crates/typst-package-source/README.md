...
`typst-package-source` (typst-syntax tier) → `typst-embed` (worlds and rendering) →
`diotypst` (Dioxus integration). `typst-embed` re-exports this crate's entire surface, so
